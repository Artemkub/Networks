    ...
    while (g_running) {
        ...
        if (msg.type == MSG_PUBLIC) {
            ...
            std::cout << "[PRIVATE] " << msg.from << " -> " << msg.to
                      << ": " << msg.text << "\n";
            // Берём g_cs только для поиска — сразу отпускаем до вызова queueSend
            pthread_mutex_lock(&g_cs);
            int targetIdx = findByName(msg.to);
            int targetSock = (targetIdx != -1 && g_clients[targetIdx] && g_clients[targetIdx]->active)
                             ? g_clients[targetIdx]->sock : -1;
            pthread_mutex_unlock(&g_cs);

            if (targetSock != -1) {
                char priv[MAX_PACKET_SIZE];
                snprintf(priv, sizeof(priv), "[Приватно] %s: %s", msg.from, msg.text);
                queueSend(targetSock, MSG_PRIVATE, msg.from, msg.to, priv);
