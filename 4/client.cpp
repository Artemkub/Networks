...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        cout << "\nОтправка структуры на сервер:" << endl;
        emp.print();
        ...
        int responseSize;
        if (!recvAll(clientSocket, (char*)&responseSize, sizeof(responseSize))) {
            cerr << "Ошибка получения размера ответа" << endl;
            break;
        }

        if (responseSize <= 0 || responseSize >= BUFFER_SIZE) {
            cerr << "Неверный размер ответа: " << responseSize << endl;
            break;
        }

        if (!recvAll(clientSocket, responseBuffer, responseSize)) {
            cerr << "Ошибка получения данных ответа" << endl;
            break;
        }

        Employee echoed;
        if (!echoed.deserialize(responseBuffer, responseSize)) {
            cerr << "Ошибка десериализации эхо-ответа" << endl;
            break;
        }

        echoed.print();

        bool match = (echoed.id == emp.id) && (echoed.name == emp.name) && (echoed.salary == emp.salary);
        cout << (match ? "Структура совпадает с отправленной." : "ВНИМАНИЕ: данные не совпадают!") << "\n" << endl;
    }
