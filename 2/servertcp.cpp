...
struct Employee {
    ...
    string getSel() const {
        if (salary > avg) {
            ...
            return "выше среднего на " + to_string(diff) + " руб.";
        }
        else if (salary < avg) {
            double diff = avg - salary;
            return "ниже среднего на " + to_string(diff) + " руб.";
        }
        else {
            return "равна средней";
        }
    }
};
...
void HandleClient(int clientSocket, string clientIP) {
    ...
    while (true) {
        int dataSize;
        int bytesRead = recvAll(clientSocket, (char*)&dataSize, sizeof(dataSize), 0);
        if (bytesRead <= 0) {
            break;
        }

        if (dataSize > 0 && dataSize < buf_size) {
            bytesRead = recvAll(clientSocket, dataBuffer, dataSize, 0);
            if (bytesRead <= 0) {
                break;
            }
            ...
            if (emp.deserialize(dataBuffer, dataSize)) {
                {
                    ...
                    cout << "\n[КЛИЕНТ " << clientIP << "] прислал:" << endl;
                    emp.print();
                }

                string analysis = emp.getSel();
                string response = "Сотрудник: " + emp.name + "\n";
                response += "Зарплата: " + to_string(emp.salary) + " руб.\n";
                response += "Анализ: " + analysis + " (средняя: " + to_string(avg) + " руб.)";
